...
public:
  ...
  */
  ...
  bool GetBits(void* Buffer, unsigned int &Size, unsigned int BitCount) {
    ...
    for (int i = 0; i < (int)m_BitmapSize; i++) {
      if (BitCount == 16) {
...
//--------------------------------------------------------------------------
bool loadImage(const std::string& fileName, GLGeometryViewer* viewer, float scale)
{
  if (viewer == NULL || fileName.length() == 0) return false;

  // load bitmap
  CBitmap bmp;
  if (!bmp.Load(fileName.c_str()))
    return false;

  float ratio = float(bmp.GetHeight()) / float(bmp.GetWidth());

  float xdim = scale;
  float ydim = ratio * scale;

  float hxdim = xdim * 0.5;
  float hydim = ydim * 0.5;

  // place points for each pixel
  for (unsigned y=0; y < bmp.GetHeight(); y++)
  {
    for (unsigned x=0; x < bmp.GetWidth(); x++)
    {
      unsigned i = y * bmp.GetWidth() + x;

      Point2D point;

      point.color = makeVector4f(float(bmp.m_BitmapData[i].Red) / 255.0, float(bmp.m_BitmapData[i].Green) / 255.0, float(bmp.m_BitmapData[i].Blue) / 255.0, float(bmp.m_BitmapData[i].Alpha) / 255.0);;
      point.canBeModified = true;
      point.size = 2.0;
      point.position = makeVector2f(xdim * (float(x)/float(bmp.GetWidth())) - hxdim, ydim * (float(y)/float(bmp.GetHeight())) - hydim);
